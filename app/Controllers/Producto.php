...
class Producto extends Home {
	...
	public function __construct() {
        ...
        $this->generosproductos = new GenerosProductoModel();
	}
	...
	public function index() {
		...
		if ($this->session->get('RolAsignado') == 'Administrador') return view('administracion/producto/index', $datos);
		else return redirect()->to(base_url());
    }

	public function listar() {
        if ($this->request->isAjax() && $this->request->getMethod() == "post") {
            $productoss = $this->productos->orderBy('NombreProducto DESC')->findAll();
            $cantidad = count($productoss);
            $datosJson = '{"data": [';
			$contador = 0;
            foreach ($productoss as $producto) {
				$contador++;
				$acciones = "<form action='".site_url('producto/editar')."' method='post'>";
                $acciones .= "<a href='".site_url('producto/generos/'.$producto['CodigoProducto'])."' class='btn btn-success'><i class='fa fa-arrow-right'></i></a>";
	...
	public function nuevo() {
		...
		if ($this->session->get('RolAsignado') == 'Administrador') return view('administracion/producto/formulario', $datos);
		else return redirect()->to(base_url());
	}
	...
	public function borrar() {
        if ($this->request->isAjax() && $this->request->getMethod() == "post") {
			$id = $this->request->getPost('id');
            if (!$id) return "error";
			$this->carritos->where('Producto', $id)->delete();
			$this->generosproductos->where('Producto', $id)->delete();
			$this->productos->where('CodigoProducto', $id)->delete();
			return "ok";
        } else return "error";
	}
    ...
    public function generos($producto) {
		...
		if ($this->session->get('RolAsignado') == 'Administrador') return view('administracion/producto/generos', $datos);
		else return redirect()->to(base_url());
	}
    ...
    public function asignar($producto) {
		...
		if ($this->session->get('RolAsignado') == 'Administrador') return view('administracion/producto/asignar', $datos);
		else return redirect()->to(base_url());
	}
	...
	public function detalle($id) {
		...
		$generosProducto = $this->generosproductos
			->join('generos', 'generosproducto.Genero=generos.CodigoGenero')
			->where('generosproducto.Producto', $producto['CodigoProducto'])
		...
		return view('administracion/producto/detalle', $datos);
	}
}